...
- (UILabel *)descriptionLabelForItemAtIndex:(NSUInteger)index{
    ...
    CGFloat rad = centerPercentage * 2 * M_PI_2;
...
//    UINavigationController *navObj=[[UINavigationController alloc]init];
//
//    proposalList * proposalListObj=[[proposalList  alloc]initWithNibName:@"proposalList" bundle:nil];
//    proposalListObj.sortByStr=@"Rejected";
//    [navObj pushViewController:proposalListObj animated:YES];
...
- (CGFloat) findPercentageOfAngleInCircle:(CGPoint)center fromPoint:(CGPoint)reference{
    ...
    CGFloat percentage = (angleOfLine + M_PI_2/2)/(2 * M_PI_2);
...
- (UIImageView*)drawInflexion:(CGFloat)size center:(CGPoint)center andColor:(UIColor*)color
{
    ...
    CGContextAddArc(context, size/2, size/ 2, size/2, 0, M_PI_2*2, YES);
