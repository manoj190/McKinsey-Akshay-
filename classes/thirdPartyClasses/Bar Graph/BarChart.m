...
- (void)createYAxisLine{
    ...
    int gridYCount = (int)[[[self.barDataArray objectAtIndex:0] yAxisArray] count];
...
- (void)createGraph{
    ...
    for (BarChartDataRenderer *barData in self.barDataArray) {
        ...
        for (int i = 0; i < barData.yAxisArray.count; i++){
            ...
            [shapeLayer setStrokeColor:[[[_colorArray objectAtIndex:i] objectForKey:@"COLOR"] CGColor]];
            [shapeLayer setFillColor:[[[_colorArray objectAtIndex:i] objectForKey:@"COLOR"] CGColor]];
            ...
            [fillColorAnimation setToValue:(id)([[[_colorArray objectAtIndex:i] objectForKey:@"COLOR"] CGColor])];
...
- (UIBezierPath *)drawBarPathWithStartPoint:(CGPoint)startPoint endPoint:(CGPoint)endPoint{
    UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake(startPoint.x+20, startPoint.y, endPoint.x - startPoint.x, endPoint.y - startPoint.y)];
